...
export async function getDatabase(deleteDbOnStart: boolean, databaseType: string, maxResultForQuery: number): Promise<Database | null> {
    if (databaseType === "ACEBASE") {
        ...
        const inMemoryDb = new InMemoryDatabase(maxResultForQuery);
