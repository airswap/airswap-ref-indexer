...
describe("Order service", () => {
    ...
    describe('Get orders', () => {
        test("get all", async () => {
            ...
            const result = await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).getOrders({});
        ...
        test("get by hash", async () => {
            ...
            const result = await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).getOrders({ hash: "aze" });
        ...
        test("get by filters", async () => {
            ...
            const result = await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).getOrders(body);
    ...
    describe("Add Order", () => {
        test("Add order nominal & broadcast", async () => {
            ...
            await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).addOrder(order);

            expect(fakeDb.generateHash).toHaveBeenCalledTimes(1);
            expect(fakeDb.orderExists).toHaveBeenCalledWith("a");
            expect(fakeDb.addOrder).toHaveBeenCalledWith(expected);
            expect(fakeWeb3SwapClient.connectToChain).toHaveBeenCalledWith(5); // @todo change that
        });
        ...
        test("Add order missing data", async () => {
            ...
            await expect(async () => {
                await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).addOrder(orderMissingExpiry)
            }).rejects.toThrow();
        ...
        test("Add order invalid data", async () => {
            ...
            await expect(async () => {
                await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).addOrder(orderBadValueSenderAmount)
            }).rejects.toThrow();
            await expect(async () => {
                await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).addOrder(orderBadValueSignerAmount)
            }).rejects.toThrow("Number fields are incorrect");
        ...
        test("Add order invalid date", async () => {
            ...
            await expect(async () => {
                await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).addOrder(orderDateNotInRange)
            }).rejects.toThrow("Invalid expiry date");
        ...
        test("Missing order", async () => {
            await expect(async () => {
                await new OrderService(fakeDb as Database, fakeWeb3SwapClient as Web3SwapERC20Client).addOrder({})
            }).rejects.toThrow("No body");
